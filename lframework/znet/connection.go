...
func (c *Connection) StartReader() {
	...
	for {
		...
		default:
			...
			msg.SetData(data)
			//fmt.Println("[Receive Msg] : msgID = " , msg.GetMsgID() , " msgSub = " , msg.GetMsgSub() , " msgDataLength = " , msg.GetDataLen())
			//得到当前客户端请求的Request数据
			req := Request{
				conn: c,
				msg:  msg,
...
func (c *Connection) SendMsg(msgID uint32, msgSub uint32, data []byte) error {
	c.RLock()
	defer c.RUnlock()
	if c.isClosed == true {
	...
	msg, err := dp.Pack(NewMsgPackage(msgID, msgSub, data))
	if err != nil {
...
func (c *Connection) SendBuffMsg(msgID uint32, msgSub uint32, data []byte) error {
	c.RLock()
	defer c.RUnlock()
	if c.isClosed == true {
